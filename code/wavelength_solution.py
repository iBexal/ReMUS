...
def apply_saved_solution(orders, saved, white=None, reference=None, n_pixels=None,
                         max_shift_pixels=60.0, verify=True, verbose=True):
    """Put a saved master solution onto a new set of traces and a new arc.

    The whole reuse path in one call:

      1. identify each trace by its position on the detector, so the trace
         list does not have to match the master's;
      2. register the new arc against the master's, order by order, and
         take a robust shift along the dispersion;
      3. attach the shifted wavelength axis to every order;
      4. if given the atlas, check the result against it rather than
         assuming step 2 worked.

    reference : ReferenceLines, optional. Supply it and the applied solution
        is verified against real wavelengths; leave it out and you only know
        the two arcs resemble each other.

    Returns (shift, scatter, report or None).
    """
    solution = saved["solution"]
    n_pixels = n_pixels or solution.n_pixels

    n_numbered, spatial_shift = assign_order_numbers_from_saved(orders, saved, white=white,
                                                                verbose=verbose)
    if n_numbered == 0:
        raise RuntimeError("no trace could be matched to the master by position")

    shift, scatter, per_order, tilt = measure_arc_shift(
        orders, saved, max_shift_pixels=max_shift_pixels, verbose=verbose)

    attach_solution(orders, solution, pixel_shift=shift, quiet=not verbose)
    for order in orders:
        if order.order_number is not None:
            order.thar_pixels, order.thar_wavelengths = [], []

    report = None
    if verify and reference is not None:
        if verbose:
            print()
        report = verify_applied_solution(orders, solution, reference, shift, n_pixels,
                                         verbose=verbose)
        if verbose and not report.passed:
            print("The saved solution does not reproduce this arc. A shift cannot fix a "
                  "solution built for a different instrument state -- rebuild the master "
                  "from this night instead (BUILD_NEW_MASTER = True).")
    elif verify and verbose:
        print("No atlas supplied, so the applied solution has NOT been checked against "
              "real wavelengths -- only against the master's own arc.")
