...
Echelle wavelength calibration from a ThAr exposure.
...
The pipeline:
    ...
    6. save / reuse           save_solution(), apply_saved_solution()
...
def save_solution(path, solution, orders, report=None):
    """Save a master solution keyed by physical order number.

    Order number, not trace index: a future night may trace a different
    subset of orders, so the index is not stable but the order number is.
    The extracted ThAr spectrum of each order goes in too, because
    re-registering a later night's exposure against it is how the pixel
    shift from flexure gets measured.
    """
    payload = {
        "solution": solution,
        "orders": [
...
def assign_order_numbers_from_saved(orders, saved, max_pixel_difference=15.0):
    """Number tonight's traces by matching their spatial positions to the
    saved ones, instead of re-deriving from the grating equation.

    Where an order sits across the detector is mechanical, so it is stable
    unless the instrument has been taken apart -- and matching on it also
    catches the case where tonight's trace list is missing an order the
    master had, which index-based numbering would silently absorb.
    """
    saved_x = np.array([e["trace_center_pixel"] for e in saved["orders"]
                        if e["trace_center_pixel"] is not None])
    saved_m = np.array([e["order_number"] for e in saved["orders"]
                        if e["trace_center_pixel"] is not None])
    matched = 0
    for order in orders:
        if order.trace_center_pixel is None or len(saved_x) == 0:
            order.order_number = None
            continue
        k = int(np.argmin(np.abs(saved_x - order.trace_center_pixel)))
        diff = abs(saved_x[k] - order.trace_center_pixel)
        if diff <= max_pixel_difference:
            order.order_number = int(saved_m[k])
            matched += 1
        else:
            order.order_number = None
            print(f"  no saved order within {max_pixel_difference:.0f} px of x="
                  f"{order.trace_center_pixel:.0f} (nearest is {diff:.0f} px away) "
                  f"-- leaving this one unnumbered")
    print(f"assign_order_numbers_from_saved: matched {matched}/{len(orders)} orders by position")
    return matched


def apply_saved_solution(orders, saved, max_shift_pixels=40.0, verbose=True):
    """Reuse a master solution on a later night, correcting only for the
    rigid pixel shift since it was built.

    Each order's new ThAr spectrum is cross-correlated against the saved
    one; the per-order shifts should agree, and the median of them is
    applied. Their scatter is the diagnostic: a few tenths of a pixel means
    ordinary flexure and this is valid, while orders disagreeing by pixels
    means the dispersion itself has changed and a shift cannot express it --
    rebuild the master instead.
    """
    ...
    for order in orders:
        ...
        if m is None or m not in by_number or order.thar_spectrum is None:
            continue
        ref = np.asarray(by_number[m]["reference_thar_spectrum"], float)
        new = np.asarray(order.thar_spectrum, float)
        n = min(len(ref), len(new))
        if n < 100:
            continue
        a = np.nan_to_num(ref[:n] - median_filter(ref[:n], 201))
        b = np.nan_to_num(new[:n] - median_filter(new[:n], 201))
        if a.std() <= 0 or b.std() <= 0:
            continue
        a /= a.std()
        b /= b.std()
        lags = np.arange(-int(max_shift_pixels), int(max_shift_pixels) + 1)
        ...
        if 0 < k < len(cc) - 1:
            ...
            sub = 0.0
        shifts.append(lags[k] + sub)
        numbers.append(m)

    if not shifts:
        raise RuntimeError("no order could be registered against the saved solution")

    shifts = np.array(shifts)
    shift = float(np.median(shifts))
    scatter = float(1.4826 * np.median(np.abs(shifts - shift)))
    if verbose:
        print(f"apply_saved_solution: pixel shift {shift:+.2f} px "
              f"(scatter between {len(shifts)} orders {scatter:.2f} px)")
        if scatter > 1.0:
            print("  WARNING: orders disagree on the shift by more than a pixel. The "
                  "dispersion itself has probably changed -- rebuild the master solution "
                  "rather than trusting this.")

    for order in orders:
        if order.order_number is not None and order.order_number in by_number:
            order.wavelength_poly = OrderWavelength(solution, order.order_number,
                                                    pixel_shift=shift)
